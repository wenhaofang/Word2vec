...
class CBOW_HS_Loader():
    ...
    def build_datas(self, ids_list, max_window_size, neg_sample_size):
        ...
        for ids in ids_list:
            ...
            for center_idx in range(len(ids)):
                ...
                pos_words = self.hufsampling(center_word, mode = 'pos')
                neg_words = self.hufsampling(center_word, mode = 'neg')
    ...
    def __next__(self):
        if  self.cur_batch < self.batch_num:
            ...
            for src_words, pos_words, neg_words in batch_data:
...
def get_loader(option):
    ...
    return CBOW_HS_Loader(
        ...
        os.path.join(option.targets_path, 'CBOW_HS_' + option.dataset_name + '.cache'),
        option.use_cache
    )
